...
public class p11_line_chart extends AppCompatActivity {
    ...
    final String graph_title = "Graph Title: Revenue, Expenses, and Profits over Time. "; // Title of the graph
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mv.setImageResource(R.drawable.p11_line_chart);
        ...
        sfg.setOnFingerGestureListener(new SimpleFingerGestures_Mod.OnFingerGestureListener() {
            ...
            public boolean onSwipeUp(int fingers, long gestureDuration, double gestureDistance) {
                if (fingers == 1) {
                    ...
                    Intent back_to_graph_selector = new Intent(p11_line_chart.this, GraphSelector.class);
...
//                            }
                            ...
                            if (!intersection_sound_is_playing) {
                                ...
                                if (tapped_X_percent < 0.5 && tapped_Y_percent > 0.5) { // Bottom left intersection point
//                                    tts.speak("Intersection: Revenue, Expenses. X " + Math.round(tapped_X_location) + ", Y " + Math.round(tapped_Y_location), TextToSpeech.QUEUE_FLUSH, null);
                                    tts.speak("Intersection: Revenue, Expenses.", TextToSpeech.QUEUE_FLUSH, null);
                                }
                                else if (tapped_X_percent > 0.5 && tapped_Y_percent > 0.5) { // Bottom right intersection point
//                                    tts.speak("Intersection: Revenue, Profits. X " + Math.round(tapped_X_location) + ", Y " + Math.round(tapped_Y_location), TextToSpeech.QUEUE_FLUSH, null);
                                    tts.speak("Intersection: Revenue, Profits.", TextToSpeech.QUEUE_FLUSH, null);
                                }
                                else if (tapped_X_percent > 0.5 && tapped_Y_percent < 0.5) { // Top right intersection point
//                                    tts.speak("Intersection: Expenses, Profits. X " + Math.round(tapped_X_location) + ", Y " + Math.round(tapped_Y_location), TextToSpeech.QUEUE_FLUSH, null);
                                    tts.speak("Intersection: Expenses, Profits.", TextToSpeech.QUEUE_FLUSH, null);
                                }
                            }
                        } else {
        ...
        final Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.HONEYCOMB_MR2) { // DO NOT remove this "unnecessary" line, otherwise the app won't work on Lenovo tablets!
                    if (ContextCompat.checkSelfPermission(p11_line_chart.this,Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                        ActivityCompat.requestPermissions(p11_line_chart.this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE},1);
                    }
                }
                else {
