...
public class s21_line_chart extends AppCompatActivity {
    ...
    final String graph_title = "Graph Title: Daily Internet Traffic for Three Websites. "; // Title of the graph
    final String graph_x_axis_title = "X Axis Title: Day. "; // Title of the X axis
    final String graph_y_axis_title = "Y Axis Title: Users (thousands). "; // Title of the Y axis
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mv.setImageResource(R.drawable.s21_line_chart);
        ...
        sfg.setOnFingerGestureListener(new SimpleFingerGestures_Mod.OnFingerGestureListener() {
            ...
            public boolean onSwipeUp(int fingers, long gestureDuration, double gestureDistance) {
                if (fingers == 1) {
                    ...
                    Intent back_to_graph_selector = new Intent(s21_line_chart.this, GraphSelector.class);
...
//                                    tts.speak("Intersection: Website 1, Website 3. X " + Math.round(tapped_X_location) + ", Y " + Math.round(tapped_Y_location), TextToSpeech.QUEUE_FLUSH, null);
                                    tts.speak("Intersection: Website 1, Website 3.", TextToSpeech.QUEUE_FLUSH, null);
                                }
                                else if (tapped_X_percent > 0.5 && tapped_Y_percent < 0.5) { // Top right intersection point
//                                    tts.speak("Intersection: Website 1, Website 2. X " + Math.round(tapped_X_location) + ", Y " + Math.round(tapped_Y_location), TextToSpeech.QUEUE_FLUSH, null);
                                    tts.speak("Intersection: Website 1, Website 2.", TextToSpeech.QUEUE_FLUSH, null);
                                }
                                else if (tapped_X_percent > 0.5 && tapped_Y_percent > 0.5) { // Bottom right intersection point
//                                    tts.speak("Intersection: Website 2, Website 3. X " + Math.round(tapped_X_location) + ", Y " + Math.round(tapped_Y_location), TextToSpeech.QUEUE_FLUSH, null);
                                    tts.speak("Intersection: Website 2, Website 3.", TextToSpeech.QUEUE_FLUSH, null);
                                }
                            }
                        } else {
        ...
        final Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.HONEYCOMB_MR2) { // DO NOT remove this "unnecessary" line, otherwise the app won't work on Lenovo tablets!
                    if (ContextCompat.checkSelfPermission(s21_line_chart.this,Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                        ActivityCompat.requestPermissions(s21_line_chart.this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE},1);
                    }
                }
                else {
